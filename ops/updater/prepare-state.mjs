// Internal worker: writes only the updater's marked, isolated prepared state copy.
...
try {
    ...
    if (!roots.some(root => home.startsWith(root + path.sep)) || marker.schema !== 1 || marker.home !== home
        || marker.purpose !== 'isolated-update-test' || !path.basename(transaction).startsWith('review-')
        || path.resolve(app) !== path.join(transaction, 'source/app')) throw new Error('Marked isolated transaction required');
