...
const pythonId = value => typeof value === 'string' && /^[A-Za-z0-9][A-Za-z0-9_-]{0,127}$/.test(value);
const iso = value => {
    if (!Number.isFinite(value) || value < 0) throw new Error('Invalid Python timestamp');
...
async function namespace(state, name) {
    const result = new Map();
    for (const entry of await files(path.join(state, name))) {
        if (!entry.isFile() || !entry.name.endsWith('.json')) throw new Error('Unsupported Python namespace entry: ' + name);
        const item = JSON.parse((await regular(path.join(state, name, entry.name))).toString('utf8'));
        if (!record(item) || !pythonId(item.id) || entry.name !== item.id + '.json' || result.has(item.id)) throw new Error('Invalid Python record identity: ' + name);
...
export async function convertPythonState(state, app, { hermesModel = null, legacyModel = null, legacyApp = null, legacyWeb = 'frontend' } = {}) {
    ...
    const cards = await namespace(source, 'cards');
    const books = await namespace(source, 'worldbooks');
    const productions = await namespace(source, 'productions');
    ...
    const put = (relative, value) => {
        if (plan.has(relative)) throw new Error('Duplicate migration output');
        plan.set(relative, Buffer.isBuffer(value) ? value : Buffer.from(JSON.stringify(value, null, 2) + '\n'));
    };
    ...
    for (const file of ['story_profile.json', 'profile_eras.json', 'profile_events.jsonl']) {
        try {
            ...
            if (file === 'story_profile.json') {
                ...
                if (profile.schema_version !== 1 || !['preferences', 'recent_timeline', 'shared_story_memory'].every(key => Array.isArray(profile[key]))) throw new Error('Unsupported Story Profile schema');
            } else if (file.endsWith('.jsonl')) {
                if (!bytes.toString('utf8').split('\n').filter(line => line.trim()).every(line => record(JSON.parse(line)))) throw new Error('Invalid Profile events');
            } else if (!Array.isArray(JSON.parse(bytes.toString('utf8')))) throw new Error('Invalid Profile eras');
    ...
    const encode = async card => (await codec.decode({ format: 'json', buffer: Buffer.from(JSON.stringify(card)) })).runtimeCardBuffer;
    for (const [id, card] of cards) {
        if (!record(card.profile) || !record(card.profile.identity)) throw new Error('Python card has no canonical profile: ' + id);
        const view = storyCharacterView({ ...card, persistent_status: {} });
    ...
    for (const [id, book] of books) {
        ...
        if (!Array.isArray(data.entries)) throw new Error('Invalid Python Worldbook: ' + id);
    ...
    const report = { schema: 'python-to-node-migration/v1', pythonMigration: true, modelsCalled: 0,
        cards: cards.size, worldbooks: books.size, worlds: [], warnings: [], users: [],
        profile: (await files(state)).some(item => item.name === 'story_profile.json') ? { schema: 1, preserved: true } : null };
    const charLore = [];
    for (const [id, production] of productions) {
        if (!Array.isArray(production.story) || !['active', 'archived', undefined].includes(production.status)) throw new Error('Unsupported Python production: ' + id);
        const cast = production.runtime_cast;
        if (cast && cast.schema_version !== 3) throw new Error('Unsupported Python runtime_cast schema: ' + id);
        const originals = cast?.characters || production.cards || (production.card_ids || [production.card_id]).filter(Boolean).map(key => {
            if (!cards.has(key)) throw new Error('Missing source card: ' + id);
        ...
        const context = normalizeStoryContext({ schema_version: 1, characters,
        ...
        const knowledge = (production.worldbook_ids || []).map(bookId => {
            if (!bookNames.has(bookId)) throw new Error('Missing Python worldbook: ' + id);
            const sourceBook = books.get(bookId);
            if (sourceBook.owner_production_id && sourceBook.owner_production_id !== id) throw new Error('Python worldbook is owned by another production: ' + bookId);
        ...
        const messages = production.story.map(message => {
            if (!pythonId(message.id) || seenMessages.has(message.id) || !['user', 'char'].includes(message.role) || typeof message.text !== 'string') throw new Error('Invalid Python message: ' + id);
            ...
            if (!Array.isArray(swipes) || !swipes.every(item => typeof item === 'string') || !Number.isInteger(selected)
                || selected < 0 || selected >= swipes.length || swipes[selected] !== message.text) throw new Error('Python message alternative mismatch: ' + id);
        ...
        if (production.ui) {
            ...
            ui = normalizeWorldTheme(ui);
        }
        const manifest = validateWorldManifest({ schema_version: 2, world_id: id, revision: 0, name, persona, story_context: context,
            ...
            updated_at: iso(Math.max(production.created_at, ...production.story.map(message => message.ts))) });
        ...
        if (old && ['timeline', 'facts', 'open_threads', 'objects', 'secrets', 'style_notes'].some(key => old[key]?.length)) {
            ...
            if (valid) {
                ...
                normalizeLedger(rawLedger, {}, Object.keys(storyEntityBindings(context, persona.name)));
    ...
    if (!Array.isArray(modelConfigs.configs)) throw new Error('Invalid Python model configuration');
    const selectedLegacy = String(modelConfigs.active || 'builtin');
    if (selectedLegacy.startsWith('clawling:') && !legacyModel) throw new Error('Selected Python built-in provider configuration is missing');
    ...
    for (const config of configs) {
        if (!pythonId(config.id) || profiles.some(profile => profile.id === config.id)
            || !['base', 'key', 'model', 'name'].every(key => typeof config[key] === 'string' && config[key])) throw new Error('Invalid custom model configuration');
        const url = new URL(config.base);
        if (!['https:', 'http:'].includes(url.protocol) || url.username || url.password) throw new Error('Invalid model endpoint');
    ...
    if (!isLegacySelected && !profiles.some(profile => profile.id === selected)) throw new Error('Selected Python custom model is missing');
    ...
    report.users.push({ user: 'default-user', before: 0, after: productions.size, active: [...productions.keys()] });
    // No active output is written until every source record and destination passes preflight.
