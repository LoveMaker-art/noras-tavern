...
class GreetingOrderTests(unittest.TestCase):
    ...
    def test_runtime_starts_before_unbound_greeting_wait(self):
        calls = []
        with patch.object(integration, "start_runtime", side_effect=lambda _: calls.append("runtime")), \
    ...
    def test_success_order_runtime_greeting_apps_verified_notice(self):
        ...
        with patch.object(integration, "start_runtime", side_effect=lambda _: calls.append("runtime")), \
             ...
             patch.object(integration, "ensure", side_effect=lambda _: calls.append("apps") or {"status": "updated"}), \
             patch.object(integration, "verified_entry", side_effect=lambda _: calls.append("verify") or {"status": "ready"}), \
             patch.object(notice, "notify_ready", side_effect=lambda *_: calls.append("notice") or {"status": "sent"}):
    ...
    def test_reconcile_cannot_bypass_greeting_gate(self):
        with patch.object(integration, "authenticate", return_value={"user_id": "owner", "instance_id": "instance"}), \
