...
Use after authorization to test on a disposable copy. Test-port mode requires a
marked temporary home; ports 8799 and 8809 are reserved. The production CLI and
the test harness share `CleanUpdater`; there is no separate migration algorithm.
...
2. Pause the reviewed source under a maintenance window. Copy its complete project state. Convert only that
...
7. On failure restore original trees and the recorded running/offline condition.
   This fixture starts with Python offline and must keep it offline on rollback;
   blindly starting its main() could trigger billable backlog.
...
- `installed-awaiting-hermes-reload` proves only the reported checks. External
  writer coordination, real user-data reconciliation, Liveware/UI acceptance and
