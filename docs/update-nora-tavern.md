...
桌面启动器本身和它管理的诺拉、酒馆系统是两层更新。**普通内容更新不需要重新下载 DMG 或 EXE；提示需要更换启动器，或希望获得新版启动器自身的改进时，再下载安装包。**

v2.3.2 改用了统一更新器，要求启动器至少为 **0.3.4**。使用旧启动器的用户按下面步骤更换一次；保留原数据，之后普通内容更新仍从启动器中完成。

当前正式包为 **酒馆系统 2.3.3 / 启动器 1.0.0**。1.0.0 增加完整安装诊断、自动日志轮转及本地接入标识，这些属于桌面程序改动，单独更新酒馆内容不会替换旧启动器。[选择对应平台下载](../README.md#下载安装包)，安装后再检查已安装系统的版本。
