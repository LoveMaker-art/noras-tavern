...
| Liveware 身份、隧道、入口注册和重试 | `ops/updater/liveware_integration.py` | 调用公共启动钩子 |
| 开场白确认和经过校验的入口消息 | `ops/updater/liveware_notice.py` | 由公共注册流程调用 |
| ClawChat 开场白顺序补丁 | `ops/updater/clawchat_greeting_patch.py` 及同目录 `.patch` | 构建时预应用，安装时校验 |
| Nora MCP 配置生成 | `ops/updater/update.py::render_mcp` | 首装调用同一函数，显式传入端口 |
| 酒馆模型凭据及设置写入 | `app/native_model_config.py::NativeSettingsClient.configure` | 首次配置策略先判断是否允许同步，再调用公共写入函数 |
| 本地文件锁 | `ops/updater/runtime_lock.py` | 安装、注册和服务停止使用同一跨平台锁实现 |

Hermes 网关的进程管理属于 `launcher_services.py`，不是酒馆进程管理的第二套实现。
完整系统更新负责协调 Hermes 与 Tavern 的备份、安装和回滚，不代替 Tavern 单体更新器的内部业务。

## 两种安装上下文

- 普通 Hermes：公共脚本保留 `HERMES_HOME` / `TAVERN_DATA_ROOT` 和原有默认目录；AGENTS 保留用户内容，只更新受管段落。
- 启动器专属 Nora：显式传入隔离的 Hermes、Tavern 目录和端口；专属安装执行完整 AGENTS 替换，并只保留一份前版备份。
...
3. 公共注册任务等待已发送的开场白，再校验当前身份、注册入口、验证入口并发送通知。

本地服务就绪不等于 ClawChat 入口已注册。等待首次对话时，本地酒馆仍可使用，界面保留入口待就绪提示；入口 URL 只能通过公共 `verified_entry` 获取。
