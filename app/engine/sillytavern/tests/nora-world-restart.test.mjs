...
async function fixture(t, { fail = () => {} } = {}) {
    ...
    let context = editStoryCharacter(createStoryContext({ name: 'Player' }), { id: 'actor:a', operation: 'create',
    ...
    const card = { spec: 'chara_card_v3', spec_version: '3.0', data: {
        ...
        extensions: { nora_world: { story_context: context } },
    ...
    const command = await stageCardBuffer({ buffer: Buffer.from(JSON.stringify(card)), originalName: 'fixture.json', sourceType: 'character-card',
        idempotencyKey: 'test:original', persona: { name: 'Player', description: 'Author persona' }, worldName: 'Original', stagingRoot });
