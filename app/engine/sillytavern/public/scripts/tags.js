 ...
 * @param {Array<Object>} entities List of entities for display, consisting of tags and characters.
...
/**
 * Gets the character key by searching based on the name or avatar. If none can be found, a toastr will be shown and null returned.
...
export function searchCharByName(charName, { suppressLogging = false } = {}) {
    const entity = charName ? findChar({ name: charName }) : characters[this_chid];
...
function onTagListHintClick() {
    ...
    setTagFilterVisibility(tag_filter_type.character, isSelected);
    console.debug('show_tag_filters:', isSelected);
}
