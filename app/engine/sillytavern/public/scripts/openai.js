...
import { LEDGER_SOURCE, containsLedger } from './nora-story-ledger/history.js';
import { prepareLedgerHistory, rememberLedgerPrompt, ledgerPromptPlan, ledgerPromptValid, acknowledgeLedger } from './nora-story-ledger/client.js';
...
import { deleteSecret, SECRET_KEYS, secret_state, writeSecret } from './secrets.js';
...
import { countTokensOpenAIAsync, getTokenizerModel, prefetchTokenCountsOpenAI } from './tokenizers.js';
import { isMobile } from './RossAscends-mods.js';
import { saveLogprobsForActiveMessage } from './logprobs-state.js';
...
export function hasCustomChatCompletionApiKey() {
    ...
    return Array.isArray(secrets)
        && secrets.some(secret => secret?.active !== false && String(secret?.value || '').trim());
}
...
function setOpenAIMessages(chat) {
    ...
    for (let i = chat.length - 1; i >= 0; i--) {
        ...
        // For sendas commands, prepend the message author.
        ...
        if (chat[j][LEDGER_SOURCE]) messages[i][LEDGER_SOURCE] = chat[j][LEDGER_SOURCE];
        j++;
    }
...
function setupChatCompletionPromptManager(openAiSettings) {
    ...
    if (promptManager) {
        if (promptManager.containerElement) {
    ...
    if (document.getElementById(configuration.containerIdentifier)) {
        promptManager.init(configuration, openAiSettings);
        promptManager.render(false);
    } else {
        promptManager.initState(configuration, openAiSettings);
    }
...
async function populateChatHistory(messages, prompts, chatCompletion, type = null, cyclePrompt = null) {
    ...
    const preparedChatPool = chatPool.map((chatPrompt, index) => {
        const prompt = new Prompt(chatPrompt);
        prompt.identifier = `chatHistory-${messages.length - index}`;
        const preparedPrompt = promptManager.preparePrompt(prompt);
        const tokenMessages = [];
        const baseMessage = { role: preparedPrompt.role, content: preparedPrompt.content };
        if (typeof preparedPrompt.content === 'string' && preparedPrompt.content.length > 0) {
            tokenMessages.push(baseMessage);
        }
        if (promptManager.serviceSettings.names_behavior === character_names_behavior.COMPLETION && prompt.name) {
            ...
            tokenMessages.push({ ...baseMessage, name });
        }
        return { chatPrompt, prompt, preparedPrompt, tokenMessages };
    });
    await prefetchTokenCountsOpenAI(preparedChatPool.flatMap(item => item.tokenMessages));

    for (let index = 0; index < preparedChatPool.length; index++) {
        const { chatPrompt, prompt, preparedPrompt } = preparedChatPool[index];
        const chatMessage = await Message.fromPromptAsync(preparedPrompt);
...
async function populateDialogueExamples(prompts, chatCompletion, messageExamples) {
    ...
    if (Array.isArray(messageExamples) && messageExamples.length) {
        await prefetchTokenCountsOpenAI([
            { role: 'system', content: substituteParams(oai_settings.new_example_chat_prompt) },
            ...messageExamples.flatMap(dialogue => dialogue.flatMap(prompt => [
                { role: 'system', content: prompt.content || '' },
                { role: 'system', content: prompt.content || '', name: prompt.name },
            ])),
        ]);
...
async function populateChatCompletion(prompts, chatCompletion, { bias, quietPrompt, quietImage, type, cyclePrompt, messages, messageExamples }) {
    // Warm known prompt counts together before the ordered budget/assembly pass.
    // This does not change prompt order, truncation, extension events or token math.
    await prefetchTokenCountsOpenAI([
        ...prompts.collection.filter(prompt => typeof prompt.content === 'string' && prompt.content.length > 0)
            .map(prompt => ({ role: prompt.role || 'system', content: prompt.content })),
        { role: 'system', content: substituteParams(oai_settings.new_chat_prompt) },
        { role: 'user', content: oai_settings.send_if_empty },
...
export async function prepareOpenAIMessages({
    ...
    messages,
    messageExamples,
}, dryRun, { skipStoryLedger = false } = {}) {
    // Without a character selected, there is no way to accurately calculate tokens
    if (!promptManager.activeCharacter && dryRun) return [null, false];

    const original = { name2, charDescription, charPersonality, scenario, worldInfoBefore, worldInfoAfter, bias,
        type, quietPrompt, quietImage, extensionPrompts, cyclePrompt, systemPromptOverride, jailbreakPromptOverride,
        messages: messages.map(message => ({ ...message })), messageExamples };
    const ledger = skipStoryLedger ? null : await prepareLedgerHistory(messages, { dryRun, type, source: oai_settings.chat_completion_source });
    const fallback = () => prepareOpenAIMessages(original, dryRun, { skipStoryLedger: true });
    if (ledger) {
        messages = ledger.messages;
        // Worldbook activation already scanned the FULL raw history. The ledger
        // now joins the mandatory prompt budget, not the prunable chat tail.
        worldInfoBefore = `${worldInfoBefore || ''}\n${ledger.text}`;
    }

    const chatCompletion = new ChatCompletion();
    let ledgerAssemblyFailed = false;
    ...
    if (ledgerAssemblyFailed) return fallback();
    ...
    if (ledger) {
        if (!containsLedger(chat, ledger.text)) return fallback();
        rememberLedgerPrompt(chat, ledger, fallback);
    }
...
async function sendOpenAIRequest(type, messages, signal, { jsonSchema = null } = {}) {
    ...
    const ledger = ledgerPromptPlan(messages);
    if (ledger && !ledgerPromptValid(generate_data.messages, ledger)) {
        const [raw] = await ledger.fallback();
        return sendOpenAIRequest(type, raw, signal, { jsonSchema });
    }
    if (ledger) generate_data.nora_story_ledger = { ...ledger.scope, recordId: ledger.record.id };
    ...
    if (ledger && response.headers.get('X-Nora-Ledger-Activated') === ledger.record.id) acknowledgeLedger(ledger);
    if (stream) {
 ...
 * @returns {import('./logprobs-state.js').TokenLogprobs[] | null} converted logprobs
 */
...
function parseOpenAIChatLogprobs(logprobs) {
    ...
    return content.map(({ token, logprob, top_logprobs = [] }) => {
        ...
        /** @type {import('./logprobs-state.js').Candidate[]} */
 ...
 */
function loadOpenAISettings(data, settings, { applyLegacyUi = true } = {}) {
    const settingNameList = data.openai_setting_names ?? [];
    openai_settings = (data.openai_settings ?? []).map(item => typeof item === 'string' ? JSON.parse(item) : item);
    const settingNames = {};
    settingNameList.forEach(function (item, i) {
        settingNames[item] = i;
    });
 ...
 * @param {ChatCompletionSettings} settings The settings object
 * @param {boolean} applyPreset Whether the saved preset should become active
 * @returns {Promise<void>}
 */
async function saveOpenAIPreset(name, settings, applyPreset = true) {
    ...
    if (savePresetSettings.ok) {
        ...
        if (applyPreset) {
            await applyOpenAIPreset(data.name);
        }
    } else {
...
/**
 * Apply an OpenAI preset directly to runtime state.
 * @param {string} presetName Preset name
 * @returns {Promise<void>}
 */
export async function applyOpenAIPreset(presetName) {
    const presetNameBefore = oai_settings.preset_settings_openai;
    const presetIndex = openai_setting_names[presetName];
    if (presetIndex === undefined) {
        throw new Error(`OpenAI preset not found: ${presetName}`);
    }
    ...
    for (const [key, [, setting, , isConnection]] of Object.entries(settingsToUpdate)) {
...
// Legacy settings UI adapter. Nora calls applyOpenAIPreset directly.
function onSettingsPresetChange() {
    const presetName = $('#settings_preset_openai').find(':selected').text();
    void applyOpenAIPreset(presetName);
}
...
/**
 * Applies and connects a custom OpenAI-compatible chat-completion profile.
...
export async function configureCustomChatCompletion({ url, model, apiKey = '', context, maxTokens }) {
    ...
    const sourceChanged = oai_settings.chat_completion_source !== chat_completion_sources.CUSTOM;

    if (String(apiKey).trim()) {
        await writeSecret(SECRET_KEYS.CUSTOM, String(apiKey).trim());
    }

    cancelStatusCheck('Custom chat completion profile changed');
    model_list = [];
 ...
 * @returns {Promise<void>}
 */
export async function clearCustomChatCompletion() {
    cancelStatusCheck('Custom chat completion profile removed');
    const savedSecrets = secret_state[SECRET_KEYS.CUSTOM];
    if (Array.isArray(savedSecrets) && savedSecrets.length) {
        for (const secret of [...savedSecrets]) {
            await deleteSecret(SECRET_KEYS.CUSTOM, secret?.id);
        }
    } else {
        await deleteSecret(SECRET_KEYS.CUSTOM);
    }
    if (hasCustomChatCompletionApiKey()) {
        throw new Error('The stored custom model key could not be removed.');
    }
    model_list = [];
 ...
 * Proxy stuff
 */
export function loadProxyPresets(settings, { applyLegacyUi = true } = {}) {
...
let openAiRuntimeInitialized = false;
let openAiLegacyUiInitialized = false;

export function initOpenAIRuntime() {
    if (openAiRuntimeInitialized) return;
    openAiRuntimeInitialized = true;
...
export function initOpenAI() {
    initOpenAIRuntime();
    if (openAiLegacyUiInitialized) return;
    openAiLegacyUiInitialized = true;
