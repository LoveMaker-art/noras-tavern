...
import { debounce_timeout, persona_description_positions } from './constants.js';
...
/** @type {FilterHelper | undefined} Filter helper for the persona list */
export let personasFilter;

function getPersonasFilter() {
    return personasFilter ??= new FilterHelper(debounce(getUserAvatars, debounce_timeout.quick));
}
 ...
 */
export async function setUserAvatar(imgfile, { toastPersonaNameChange = true } = {}) {
 ...
 */
const countPersonaDescriptionTokens = debounce(async (value = null) => {
    const description = typeof value === 'string' ? value : String($('#persona_description').val());
    ...
    if (targetedChar) {
        customButtons.push({
            ...
            action: () => {
                ...
                updatePersonaConnectionsAvatarList();
                if (power_user.persona_show_notifications) {
                    const name = characters.find(character => character.avatar === targetedChar.id)?.name;
...
/**
 * Updates the active persona description.
 * @param {string} value Persona description
 * @param {{syncUi?: boolean}} options Update options
 */
 ...
 */
export async function onPersonaLoreButtonClick({ shiftKey, altKey }) {
    const personaName = power_user.personas[user_avatar];
...
export async function onPersonaDescriptionPositionInput() {
...
export function onBackupPersonas() {
...
export async function onPersonasRestoreInput(e) {
