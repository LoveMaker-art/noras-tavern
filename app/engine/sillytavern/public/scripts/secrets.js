...
import { getCurrentUserHandle } from './workspace-user.js';
 ...
 * @param {{syncUi?: boolean}} [options] Whether to synchronize legacy secret inputs
 * @returns {Promise<void>}
 */
export async function readSecretState(options) {
    const { syncUi = true } = options ?? {};
    try {
        const bundledState = globalThis.__NORA_SECRET_STATE_PROMISE__
            ? await globalThis.__NORA_SECRET_STATE_PROMISE__.catch(() => null)
            : null;
        if (bundledState && typeof bundledState === 'object') {
            secret_state = bundledState;
            if (syncUi) {
                updateSecretDisplay();
