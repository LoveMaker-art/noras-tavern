...
class RegexPresetManager {
    ...
    /**
     * Returns the selected preset from extension state without consulting the UI.
     * @returns {string|null} The selected preset ID
     */
    getSelectedPresetId() {
        return extension_settings.regex_presets?.find(preset => preset.isSelected)?.id ?? null;
    }

    /**
     * Applies and selects a preset through extension state.
     * @param {string} presetId The preset ID to select
     * @returns {Promise<boolean>} Whether the preset was selected
     */
    async selectPreset(presetId) {
        ...
        extension_settings.regex_presets.forEach(item => { item.isSelected = item.id === presetId; });
        saveSettingsDebounced();
        this.updateStoredState(presetId);
        return true;
    }
    ...
    setupEventListeners() {
        ...
        this.presetSelect.addEventListener('change', async (event) => {
            ...
            await this.selectPreset(selectedPresetId);
        });
        ...
        SlashCommandParser.addCommandObject(SlashCommand.fromProps({
            ...
            callback: async (args, name) => {
                ...
                if (name) {
                    ...
                    if (foundId) {
                        await this.selectPreset(foundId);
                        if (this.presetSelect) {
                ...
                return this.getSelectedPresetId() ?? '';
            },
        ...
        if (Array.isArray(scripts) && scripts.length > 0) {
            if (!isScopedScriptsAllowed(character)) {
                ...
                if (!accountStorage.getItem(checkKey)) {
                    ...
                    const noraConfirm = globalThis.__NORA_CONFIRM_CHARACTER_REGEX__;
                    const result = typeof noraConfirm === 'function'
                        ? await noraConfirm({ characterName: character.name, scripts })
                        : await callGenericPopup(
                            await renderExtensionTemplateAsync('regex', 'embeddedScripts', {}),
                            POPUP_TYPE.CONFIRM,
                            '',
                        );
