...
import { adoptWaitingReasoningView, initializeReasoningView, WAITING_REASONING_TITLE } from './nora-compat/reasoning-view.js';
...
export function isReasoningAutoParseEnabled() {
    return power_user.reasoning.auto_parse || globalThis.document?.body?.classList?.contains('nora-product');
}
...
function toggleReasoningAutoExpand() {
    ...
    const noraProduct = document.body?.classList?.contains('nora-product');
    reasoningBlocks.forEach((block) => {
        if (block instanceof HTMLDetailsElement) {
            block.open = noraProduct ? false : power_user.reasoning.auto_expand;
        }
    });
}
...
export class ReasoningHandler {
    ...
    constructor(timeStarted = null) {
        ...
        // Only a live stream can own preflight presentation; history must never adopt it.
        this.isStreaming = timeStarted !== null;
    ...
    initHandleMessage(messageIdOrElement, { reset = false } = {}) {
        ...
        if (globalThis.document?.body?.classList?.contains('nora-product')) {
            initializeReasoningView(this.messageReasoningDetailsDom);
        } else if (power_user.reasoning.auto_expand && this.state !== ReasoningState.Hidden) {
    ...
    async finish(messageId) {
        ...
        if (this.state === ReasoningState.Thinking) {
            ...
            // Render the final state before extension hooks run; they must not hold the thinking UI open.
            this.updateDom(messageId);
    ...
    updateDom(messageId) {
        ...
        const nora = globalThis.document?.body?.classList?.contains('nora-product');
        const waiting = nora && this.isStreaming && this.state === ReasoningState.None
            && !this.reasoning && ['', '...', '…'].includes(trimSpaces(chat[messageId]?.mes ?? ''));
        // Pending is presentation only: it is never persisted as reasoning or timing.
        this.messageDom.classList.toggle('reasoning', waiting || this.state !== ReasoningState.None);
        if (nora) this.messageReasoningDetailsDom.hidden = !waiting && this.state === ReasoningState.None;
        ...
        setDatasetProperty(this.messageReasoningDetailsDom, 'state', waiting ? 'pending' : this.state);
        ...
        this.#updateReasoningTimeUI(waiting);
    }
    ...
    #checkDomElements(messageId) {
        ...
        if (this.messageReasoningDetailsDom === null) {
            if (this.isStreaming && globalThis.document?.body?.classList?.contains('nora-product')) {
                adoptWaitingReasoningView(this.messageDom, document);
            }
     ...
     */
    #updateReasoningTimeUI(waiting = false) {
        const element = this.messageReasoningHeaderDom;
        const duration = this.getDuration();
        const isNoraProduct = globalThis.document?.body?.classList?.contains('nora-product');
        let data = null;
        let title = '';
        if (waiting || this.state === ReasoningState.Thinking) {
            // State takes priority over an old duration carried by a reused/swiped message.
            element.textContent = isNoraProduct ? WAITING_REASONING_TITLE : t`Thinking...`;
            data = null;
        } else if (duration) {
            const seconds = moment.duration(duration).asSeconds();
            if (isNoraProduct) {
                const durationLabel = seconds < 1 ? seconds.toFixed(1) : String(Math.round(seconds * 10) / 10);
                element.textContent = t`已思考 ${durationLabel} 秒`;
            } else {
            ...
            data = String(seconds);
            title = isNoraProduct ? t`思考用时 ${seconds} 秒` : `${seconds} seconds`;
        } else if ([ReasoningState.Done, ReasoningState.Hidden].includes(this.state)) {
            element.textContent = isNoraProduct ? translate('已完成思考') : t`Thought for some time`;
            data = 'unknown';
        } else {
            element.textContent = isNoraProduct ? translate('思考') : t`Thought for some time`;
            data = null;
        }
...
function setReasoningEventHandlers() {
    ...
    $(document).on('click', '.mes_reasoning_header', function (e) {
        ...
        if (details.find('.mes_reasoning').is(':empty') && details.attr('data-state') !== 'pending') {
            e.preventDefault();
            return;
        }
