...
export const CONTROL_ACTIONS = Object.freeze({
    ...
    'world.setting.add': action('Append one setting to the current World private Worldbook, preserving existing entries and library originals: setting {type:constant|trigger, title, content, keys?}. Trigger requires keys. Uses the control idempotency key; inspect World revision first.', { setting: 'object', expectedRevision: 'string' }),
    'world.library.apply': action('Copy library material into the active World using input {character?:{id,operation:create,patch}, source?:{kind:card|book,name}, source_revision?:string}. Read library first; preserve profile fields and book revision. Persona uses world.update. Does not authorize scripts or model calls.', { input: 'object', expectedRevision: 'string' }),
