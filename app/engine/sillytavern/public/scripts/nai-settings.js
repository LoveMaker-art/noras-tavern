...
export function loadNovelSettings(data, settings, { applyLegacyUi = true } = {}) {
    const settingNames = data.novelai_setting_names ?? [];
    novelai_settings = (data.novelai_settings ?? []).map(item => typeof item === 'string' ? JSON.parse(item) : item);
    const presetNames = {};
