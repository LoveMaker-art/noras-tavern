...
import { applyOpenAIPreset, oai_settings, openai_setting_names, openai_settings } from './openai.js';
...
function registerPresetManagers() {
    ...
    for (const apiId of ['kobold', 'novel', 'textgenerationwebui', 'openai', 'context', 'instruct', 'sysprompt', 'reasoning']) {
        presetManagers[apiId] ??= new PresetManager(null, apiId);
    }
}
...
class PresetManager {
    ...
    findPreset(name) {
        ...
        return Array.isArray(preset_names)
            ? (preset_names.includes(name) ? name : undefined)
            : preset_names[name];
    }
     ...
     */
    getSelectedPreset() {
        return this.findPreset(this.getSelectedPresetName());
    }
    ...
    getSelectedPresetName() {
        ...
        switch (this.apiId) {
            case 'koboldhorde':
            case 'kobold':
                selected = settings.preset_settings;
                break;
            case 'novel':
                selected = settings.preset_settings_novel;
                break;
            case 'openai':
                selected = settings.preset_settings_openai;
                break;
            default:
        ...
        const names = this.getAllPresets();
    ...
    async selectPreset(value) {
        const { preset_names } = this.getPresetList();
        const name = Array.isArray(preset_names)
            ? (preset_names.includes(value) ? value : preset_names[Number(value)])
            : Object.entries(preset_names).find(([, presetValue]) => String(presetValue) === String(value))?.[0];

        if (!name) {
            throw new Error(`Preset not found for ${this.apiId}: ${value}`);
        }

        if (this.apiId === 'openai') {
            await applyOpenAIPreset(name);
            return;
        }

        const { presets, settings } = this.getPresetList();
        const index = Array.isArray(preset_names) ? preset_names.indexOf(name) : preset_names[name];
        const preset = presets[index];
        if (!preset) {
            throw new Error(`Preset data not found for ${this.apiId}: ${name}`);
        }

        Object.assign(settings, structuredClone(preset));
        switch (this.apiId) {
            case 'koboldhorde':
            case 'kobold':
                ...
                break;
            case 'novel':
                settings.preset_settings_novel = name;
                break;
            default:
        ...
        saveSettingsDebounced();
        await eventSource.emit(event_types.PRESET_CHANGED, { apiId: this.apiId, name });
    }
    ...
    async updatePreset(option = { skipUpdate: false }) {
        const name = this.getSelectedPresetName();
        if (name === 'GUI' || this.getSelectedPreset() === 'gui') {
    ...
    updateList(name, preset) {
        ...
        void this.selectPreset(this.findPreset(name));
    }
    ...
    async deletePreset(name) {
        ...
        if (this.isKeyedApi()) {
            ...
            presets.splice(index, 1);
            for (const presetName of Object.keys(preset_names)) {
                if (preset_names[presetName] > index) preset_names[presetName]--;
            }
        }
