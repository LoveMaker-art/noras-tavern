...
import { shouldSuppressNoraMvuUpdateEntryForMainPrompt } from './nora-compat/mvu-world-info-policy.js';
...
/** @type {FilterHelper | undefined} */
export let worldInfoFilter;

function getWorldInfoFilter() {
    return worldInfoFilter ??= new FilterHelper(() => updateEditor());
}
 ...
 */
export function primeWorldInfoSnapshot(name, data) {
...
export async function updateWorldInfoList() {
    const result = await fetch('/api/settings/get', {
        method: 'POST',
        headers: getRequestHeaders(),
        body: JSON.stringify({ scope: 'world-info' }),
    });
...
export function sortWorldInfoEntries(data, { customSort = null } = {}) {
    ...
    if (sortRule === 'search') {
        primarySort = (a, b) => {
            const aScore = getWorldInfoFilter().getScore(FILTER_TYPES.WORLD_INFO_SEARCH, a.uid);
            const bScore = getWorldInfoFilter().getScore(FILTER_TYPES.WORLD_INFO_SEARCH, b.uid);
...
async function _save(name, data, options = {}) {
    ...
    const response = await fetch('/api/worldinfo/edit', {
        method: 'POST',
        headers: getRequestHeaders(),
        body: JSON.stringify({ name: name, data: data, expected_revision: options.expectedRevision }),
    });
    if (!response.ok) throw Object.assign(new Error(`Worldbook save failed (${response.status}).`),
        { code: response.status === 409 ? 'NORA_WORLDBOOK_REVISION_CONFLICT' : 'NORA_WORLDBOOK_SAVE_FAILED' });
    worldInfoCache.set(name, data);
...
export function initWorldInfo() {
    // Nora owns the worldbook editor; scanning and slash commands initialize through settings.
}
