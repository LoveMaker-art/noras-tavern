...
export function loadKoboldSettings(data, preset, settings, { applyLegacyUi = true } = {}) {
    const settingNames = data.koboldai_setting_names ?? [];
    koboldai_settings = (data.koboldai_settings ?? []).map(item => typeof item === 'string' ? JSON.parse(item) : item);
    const names = {};
    settingNames.forEach(function (item, i) {
        names[item] = i;
    });
    ...
    loadKoboldSettingsFromPreset(preset, applyLegacyUi);
...
function loadKoboldSettingsFromPreset(preset, applyLegacyUi = true) {
