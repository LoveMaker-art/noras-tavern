...
export function initDefaultSlashCommands() {
    ...
    SlashCommandParser.addCommandObject(SlashCommand.fromProps({
        ...
        helpString: t`Opens a chat with the character by its name`,
        aliases: ['char'],
    }));
    SlashCommandParser.addCommandObject(SlashCommand.fromProps({
        name: 'rename-char',
    ...
    SlashCommandParser.addCommandObject(SlashCommand.fromProps({
        ...
        helpString: `
        <div>
            ${t`Triggers a message generation for the current character.`}
        </div>
        <div>
...
async function triggerGenerationCallback(args, value) {
    if (args?._abortController?.noraAwaitGeneration) {
        await waitUntilCondition(() => !is_send_press, 10000, 100);
        await args._abortController.noraPrepareGeneration?.();
        if (args._abortController.signal.aborted) throw Object.assign(new Error('操作已取消。'), { name: 'AbortError' });
        $('#send_textarea').val('')[0].dispatchEvent(new Event('input', { bubbles: true })); await Generate('normal');
        return '';
    }
...
async function continueChatCallback(args, prompt) {
    if (args?._abortController?.noraAwaitGeneration) {
        ...
        await args._abortController.noraPrepareGeneration?.();
        if (args._abortController.signal.aborted) throw Object.assign(new Error('操作已取消。'), { name: 'AbortError' });
        $('#send_textarea').val('')[0].dispatchEvent(new Event('input', { bubbles: true })); await Generate('continue', prompt?.trim() ? { quiet_prompt: prompt.trim(), quietToLoud: true } : {});
        return '';
    }
    const shouldAwait = isTrueBoolean(args?.await);
...
async function swipeChatCallback(args) {
    if (args?._abortController?.noraAwaitGeneration) {
        ...
        await args._abortController.noraPrepareGeneration?.();
        if (args._abortController.signal.aborted) throw Object.assign(new Error('操作已取消。'), { name: 'AbortError' });
        await swipe(null, args?.direction === SWIPE_DIRECTION.LEFT ? SWIPE_DIRECTION.LEFT : SWIPE_DIRECTION.RIGHT, { source: SWIPE_SOURCE.SLASH_COMMAND, repeated: false });
        return '';
    }
    const shouldAwait = isTrueBoolean(args?.await);
