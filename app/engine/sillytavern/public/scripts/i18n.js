...
import { getCurrentLocale, localeData, setLocaleData, translate as translateCore } from './nora-i18n/core.js';
import { translateElement } from './nora-i18n/dom.js';
export { getCurrentLocale, addLocaleData, t } from './nora-i18n/core.js';
export { applyNoraLocale as initNoraLocales } from './nora-i18n/dom.js';

const getStaticAssetUrl = path => globalThis.__NORA_ASSET_URL__?.(path) ?? path;

const storageKey = 'language';
const localeFile = getCurrentLocale();
var langs;
...
async function getLocaleData(language) {
    ...
    const localeUrl = getStaticAssetUrl(`/locales/${language}.json`);
    const data = await fetch(localeUrl).then(response => {
        console.log(`Loading locale data from ${localeUrl}`);
        if (!response.ok) {
            return {};
        }
...
export function applyLocale(root = document) {
    ...
    const isMarkup = typeof root === 'string';
    const target = isMarkup ? new DOMParser().parseFromString(root, 'text/html') : root;
    if (target.matches?.('[data-i18n]')) translateElement(target);
    target.querySelectorAll('[data-i18n]').forEach(translateElement);
    if (isMarkup) return target.body.innerHTML;
...
export async function initLocales() {
    langs = await fetch(getStaticAssetUrl('/locales/lang.json')).then(response => response.json());
    setLocaleData(await getLocaleData(localeFile));
