...
const getStaticAssetUrl = path => /^\/scripts\//.test(path)
    ? (globalThis.__NORA_ASSET_URL__?.(path) ?? path)
    : path;
...
export async function renderTemplateAsync(templateId, templateData = {}, sanitize = true, localize = true, fullPath = false) {
    ...
    try {
        const pathToTemplate = getStaticAssetUrl(fullPath ? templateId : `/scripts/templates/${templateId}.html`);
