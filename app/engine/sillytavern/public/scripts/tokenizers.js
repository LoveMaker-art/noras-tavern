import { createTokenCountBatcher } from './token-count-batcher.js';
...
/**
 * Warms the current chat's OpenAI token cache with one server round trip.
 * Existing cache entries are omitted and duplicate messages are coalesced.
 * @param {object[]} messages Messages in the same shape used by countTokensOpenAIAsync.
 * @returns {Promise<number>} Number of newly cached message counts.
 */
const openAiTokenBatcher = createTokenCountBatcher({
    request: async (model, messages) => {
        const data = await jQuery.ajax({ async: true, type: 'POST',
            url: '/api/tokenizers/openai/count-batch?model=' + encodeURIComponent(model),
            data: JSON.stringify({ messages }), dataType: 'json', contentType: 'application/json' });
        return data?.token_counts;
    },
});

export async function prefetchTokenCountsOpenAI(messages) {
    if (!Array.isArray(messages) || !messages.length) return 0;
    const model = getTokenizerModel();
    const cacheObject = getTokenCacheObject();
    const missing = new Map();
    for (const message of messages) {
        const key = model + '-' + getStringHash(JSON.stringify(message));
        if (typeof cacheObject[key] !== 'number') missing.set(key, message);
    }
    await Promise.all([...missing].map(async ([key, message]) => {
        cacheObject[key] = await openAiTokenBatcher.count(model, message);
    }));
    if (missing.size) await saveTokenCache();
    return missing.size;
}

/** Count exactly as the legacy endpoint, without serial per-message HTTP calls. */
export async function countTokensOpenAIAsync(messages, full = false) {
    const model = getTokenizerModel();
    const cacheObject = getTokenCacheObject();
    const counts = await Promise.all((Array.isArray(messages) ? messages : [messages]).map(async message => {
        const key = model + '-' + getStringHash(JSON.stringify(message));
        if (typeof cacheObject[key] !== 'number') cacheObject[key] = await openAiTokenBatcher.count(model, message);
        return cacheObject[key];
    }));
    return counts.reduce((total, count) => total + count, -1) - ((full || model === 'claude') ? 0 : 2);
}
