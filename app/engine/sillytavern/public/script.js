...
import { cancelDebouncedMetadataSave, extension_settings, loadExtensionSettings, runGenerationInterceptors } from './scripts/extensions.js';
...
import { saveLogprobsForActiveMessage } from './scripts/logprobs-state.js';
...
import { initLocales, initNoraLocales, t } from './scripts/i18n.js';
...
import { getBackgrounds, initBackgrounds, loadBackgroundSettings, background_settings } from './scripts/backgrounds.js?v=2.5.3';
...
import { normalizeNoraReasoningMessage } from './scripts/nora-compat/reasoning-policy.js';
...
import { getSafetyChat, getSystemMessageByType, initSystemMessageCore, initSystemMessages, sendSystemMessage, system_message_types, system_messages } from './scripts/system-messages.js';
...
/**
 * Preserve the native ESM cycle ordering used by the ST runtime. Module scripts
 * execute after parsing, so this normally resolves immediately.
 */
...
// Publish the compatibility API only after every module binding has initialized.
...
const legacyOptionsButton = document.getElementById('options_button');
const legacyOptionsMenu = document.getElementById('options');
const legacyExportButton = document.getElementById('export_button');
const legacyExportMenu = document.getElementById('export_format_popup');
let optionsPopper = legacyOptionsButton && legacyOptionsMenu
    ? Popper.createPopper(legacyOptionsButton, legacyOptionsMenu, { placement: 'top-start' })
    : null;
let exportPopper = legacyExportButton && legacyExportMenu
    ? Popper.createPopper(legacyExportButton, legacyExportMenu, { placement: 'left' })
    : null;
...
async function getClientVersion() {
    try {
        const bundledVersion = globalThis.__NORA_VERSION_PROMISE__
            ? await globalThis.__NORA_VERSION_PROMISE__.catch(() => null)
            : null;
        const data = bundledVersion || await fetch('/version').then(response => response.json());
...
export const entitiesFilter = new FilterHelper(printCharactersDebounced, () => tag_map);
...
export function getRequestHeaders({ omitContentType = false } = {}) {
    ...
    const traceId = String(globalThis.__NORA_BOOT_METRICS__?.sessionId || '');
    if (/^[A-Za-z0-9:._-]{1,100}$/.test(traceId)) headers['X-Nora-Trace-Id'] = traceId;
...
$.ajaxPrefilter((options, originalOptions, xhr) => {
    ...
    const traceId = String(globalThis.__NORA_BOOT_METRICS__?.sessionId || '');
    if (/^[A-Za-z0-9:._-]{1,100}$/.test(traceId)) xhr.setRequestHeader('X-Nora-Trace-Id', traceId);
});
...
const noraBootMetrics = globalThis.__NORA_BOOT_METRICS__ ??= {
    startedAt: performance.now(),
    steps: [],
    extensions: [],
    extensionBatches: [],
};
noraBootMetrics.milestones ??= [];
noraBootMetrics.longTasks ??= [];
...
async function requestNoraChatWindow({ before, full = false } = {}) {
    ...
    const body = {
        ch_name: binding.character.name,
        file_name: binding.chatId,
        avatar_url: binding.avatar,
    };
    if (!full) {
...
export function getNoraAbsoluteMessageId(messageId) {
    ...
    return Number.isInteger(id) && matchesNoraChatWindow()
        ? noraChatWindowState.start + id
        : id;
}
...
function normalizeBootResourceName(value) {
    const raw = String(value || '');
    if (/^(?:data|blob):/i.test(raw)) return '[inline-resource]';
    try {
        return new URL(raw, location.href).pathname.slice(0, 200);
    } catch {
        return raw.split(/[?#]/, 1)[0].slice(0, 200);
    }
}

function getBootResourceSource(entry) {
    if (Number(entry.transferSize) > 0) return 'network';
    if (Number(entry.decodedBodySize) > 0) return 'cache';
    return 'unknown';
}

noraBootMetrics.milestones.push({
    name: 'main-module-evaluating',
    at: roundBootMetric(performance.now() - noraBootMetrics.startedAt),
});

function getBootResourceMetrics() {
    const origin = Number(noraBootMetrics.startedAt) || 0;
    const relative = value => roundBootMetric(Number(value) - origin);
    const validPhase = (start, end) => Number.isFinite(start) && Number.isFinite(end) && end >= start;
    return performance.getEntriesByType('resource')
        .map((entry) => ({
            name: normalizeBootResourceName(entry.name),
            initiator: entry.initiatorType,
            start: relative(entry.startTime),
            fetchStart: relative(entry.fetchStart),
            requestStart: relative(entry.requestStart),
            responseStart: relative(entry.responseStart),
            responseEnd: relative(entry.responseEnd),
            duration: roundBootMetric(entry.duration),
            queueDuration: validPhase(entry.fetchStart, entry.requestStart)
                ? roundBootMetric(entry.requestStart - entry.fetchStart) : -1,
            ttfb: validPhase(entry.requestStart, entry.responseStart) && entry.responseStart > 0
                ? roundBootMetric(entry.responseStart - entry.requestStart) : -1,
            downloadDuration: validPhase(entry.responseStart, entry.responseEnd) && entry.responseStart > 0
                ? roundBootMetric(entry.responseEnd - entry.responseStart) : -1,
            transferSize: Number(entry.transferSize) || 0,
            decodedBodySize: Number(entry.decodedBodySize) || 0,
            responseStatus: Number(entry.responseStatus) || 0,
            protocol: entry.nextHopProtocol || '',
            source: getBootResourceSource(entry),
            serverTiming: Array.from(entry.serverTiming || []).slice(0, 8)
                .map(item => `${String(item.name || '').slice(0, 40)}:${roundBootMetric(item.duration)}`).join(','),
        }))
        .sort((a, b) => a.start - b.start)
        .slice(0, 100);
}

function getBootNavigationMetrics() {
    const navigation = performance.getEntriesByType('navigation')[0];
    if (!navigation) return null;
    return {
        start: roundBootMetric(navigation.startTime),
        fetchStart: roundBootMetric(navigation.fetchStart),
        domainLookupStart: roundBootMetric(navigation.domainLookupStart),
        domainLookupEnd: roundBootMetric(navigation.domainLookupEnd),
        connectStart: roundBootMetric(navigation.connectStart),
        secureConnectionStart: roundBootMetric(navigation.secureConnectionStart),
        connectEnd: roundBootMetric(navigation.connectEnd),
        requestStart: roundBootMetric(navigation.requestStart),
        responseStart: roundBootMetric(navigation.responseStart),
        responseEnd: roundBootMetric(navigation.responseEnd),
        dnsDuration: roundBootMetric(Math.max(0, navigation.domainLookupEnd - navigation.domainLookupStart)),
        connectDuration: roundBootMetric(Math.max(0, navigation.connectEnd - navigation.connectStart)),
        ttfb: roundBootMetric(Math.max(0, navigation.responseStart - navigation.requestStart)),
        downloadDuration: roundBootMetric(Math.max(0, navigation.responseEnd - navigation.responseStart)),
        domInteractive: roundBootMetric(navigation.domInteractive),
        domContentLoadedStart: roundBootMetric(navigation.domContentLoadedEventStart),
        domContentLoaded: roundBootMetric(navigation.domContentLoadedEventEnd),
        loadEventStart: roundBootMetric(navigation.loadEventStart),
        loadEventEnd: roundBootMetric(navigation.loadEventEnd),
...
function publishNoraBootMetrics(phase, metrics) {
    let output = document.getElementById('nora-boot-metrics');
    if (!output) {
        output = document.createElement('script');
        output.id = 'nora-boot-metrics';
        output.type = 'application/json';
        document.head.append(output);
    }
    output.textContent = JSON.stringify({ phase, metrics });
}
...
async function sendNoraBootMetrics(payload) {
    if (globalThis.__NORA_CSRF_PROMISE__) {
        try {
            token = await globalThis.__NORA_CSRF_PROMISE__;
        } catch {
...
function reportNoraBootMetrics(phase) {
    const snapshot = structuredClone(noraBootMetrics);
    snapshot.steps = (snapshot.steps || []).slice(0, 120);
    snapshot.extensions = (snapshot.extensions || []).slice(0, 120);
    snapshot.extensionBatches = (snapshot.extensionBatches || []).slice(0, 120);
    snapshot.milestones = (snapshot.milestones || []).slice(0, 120);
    snapshot.longTasks = (snapshot.longTasks || []).slice(0, 120);
    const metrics = {
        ...snapshot,
        capturedAt: roundBootMetric(performance.now() - noraBootMetrics.startedAt),
        navigation: getBootNavigationMetrics(),
    ...
    noraMetricReportQueue = noraMetricReportQueue
        .catch(() => {})
        .then(() => sendNoraBootMetrics(payload));
    void noraMetricReportQueue.catch(error => console.warn('[Nora boot] Could not report metrics:', error.message));
}

globalThis.__NORA_REPORT_BOOT_METRICS__ = reportNoraBootMetrics;

async function timedBootStep(name, operation) {
    const rawStartedAt = performance.now();
    const startedAt = roundBootMetric(rawStartedAt - noraBootMetrics.startedAt);
    let status = 'ok';
    try {
...
globalThis.__NORA_TIMED_STEP__ = timedBootStep;

function timedBootSyncStep(name, operation) {
    const rawStartedAt = performance.now();
    const startedAt = roundBootMetric(rawStartedAt - noraBootMetrics.startedAt);
    let status = 'ok';
    try {
...
function waitForNoraRuntimeReady() {
    if (document.body.classList.contains('nora-runtime-ready')) return Promise.resolve();
    return new Promise(resolve => globalThis.addEventListener('nora:runtime-ready', resolve, { once: true }));
}

async function finishDeferredInitialization(criticalExtensionActivation) {
    await criticalExtensionActivation;
    const criticalExtensionsReadyAt = roundBootMetric(performance.now() - noraBootMetrics.startedAt);
    noraBootMetrics.criticalExtensionsReadyAt = criticalExtensionsReadyAt;
    document.documentElement.dataset.noraCriticalExtensionsReadyMs = String(criticalExtensionsReadyAt);
    document.body.classList.add('nora-critical-extensions-ready');
    if (extensionActivationPlan) {
        await timedBootStep('extensions-background', async () => {
            await extensionActivationPlan.activateRemaining();
            await eventSource.emit(event_types.EXTENSION_SETTINGS_LOADED);
        });
    }
    const extensionsReadyAt = roundBootMetric(performance.now() - noraBootMetrics.startedAt);
    noraBootMetrics.extensionsReadyAt = extensionsReadyAt;
    noraBootMetrics.milestones.push({ name: 'deferred-initialization-end', at: extensionsReadyAt });
    document.documentElement.dataset.noraExtensionsReadyMs = String(extensionsReadyAt);
    document.body.classList.add('nora-extensions-ready');
    console.info('[Nora boot] deferred initialization complete', { extensionsReadyAt });
    reportNoraBootMetrics('extensions-ready');
}

//MARK: firstLoadInit
async function firstLoadInit() {
    const isNoraProduct = isNoraProductMode();
    noraBootMetrics.milestones.push({
        name: 'first-load-init-start',
        at: roundBootMetric(performance.now() - noraBootMetrics.startedAt),
    });
    try {
        await timedBootStep('csrf', async () => {
            if (globalThis.__NORA_CSRF_PROMISE__) {
                token = await globalThis.__NORA_CSRF_PROMISE__;
                return;
            }
    ...
    const initLoaderHandle = {
        async hide() {
            document.getElementById('preloader')?.remove();
        },
    };

    timedBootSyncStep('bootstrap-dom', () => {
        if (isNoraProduct) {
            document.documentElement.dataset.noraAccountMode = 'single-user';
            document.getElementById('admin_button')?.remove();
    ...
    const metadataTasks = [
        timedBootStep('client-version', getClientVersion),
        timedBootStep('secrets-state', () => readSecretState({ syncUi: !isNoraProduct })),
        ...(!isNoraProduct ? [timedBootStep('secrets-ui', initSecrets)] : []),
    ];
    if (isNoraProduct) initNoraLocales();
    if (!isNoraProduct) metadataTasks.push(timedBootStep('locales', initLocales));
    await Promise.all(metadataTasks);
    timedBootSyncStep('engine-initializers', () => {
        initChatUtilities();
    ...
    await timedBootStep('preset-manager', initPresetManager);
    const settingsTasks = [timedBootStep('settings-core', () => getSettings(initLoaderHandle))];
    if (!isNoraProduct) settingsTasks.push(timedBootStep('system-messages', initSystemMessages));
    await Promise.all(settingsTasks);
    // Empty workspaces are interactive too. World usability is an outcome metric,
    // not a prerequisite for initializing the dependencies of the first send.
    const noraRuntimeReadyPromise = isNoraProduct ? waitForNoraRuntimeReady() : Promise.resolve();
    const tokenCacheInitialization = isNoraProduct
        ? noraRuntimeReadyPromise.then(() => timedBootStep('token-cache', initTokenizers))
        : Promise.resolve();
    globalThis.__NORA_TOKEN_CACHE_PROMISE__ = tokenCacheInitialization;
    const criticalExtensionActivation = extensionActivationPlan
        ? (isNoraProduct
            ? noraRuntimeReadyPromise.then(() => timedBootStep('extensions-critical', () => extensionActivationPlan.activateCritical(criticalExtensionNames)))
            : timedBootStep('extensions-critical', () => extensionActivationPlan.activateCritical(criticalExtensionNames)))
        : Promise.resolve();
    globalThis.__NORA_CRITICAL_EXTENSIONS_PROMISE__ = criticalExtensionActivation;
    globalThis.__NORA_GENERATION_PREREQUISITES_PROMISE__ = Promise.all([
        tokenCacheInitialization,
        criticalExtensionActivation,
    ]);
    if (!isNoraProduct) {
        await timedBootStep('openrouter-auth', checkOpenRouterAuth);
    }
    if (!isNoraProduct) {
        timedBootSyncStep('post-settings-ui', () => {
            initKeyboard();
            initDynamicStyles();
            initTags();
            initBookmarks();
    ...
    await timedBootStep('core-data', () => Promise.all(coreDataTasks));
    const coreReadyAt = Math.round((performance.now() - noraBootMetrics.startedAt) * 10) / 10;
    noraBootMetrics.coreReadyAt = coreReadyAt;
    document.documentElement.dataset.noraCoreReadyMs = String(coreReadyAt);
    document.body.classList.add('nora-core-ready');
    timedBootSyncStep('post-core-chat', () => {
        initBackgrounds();
        initAuthorsNote();
    });
    if (!isNoraProduct) {
        await timedBootStep('personas', initPersonas);
    }
    if (!isNoraProduct) {
        await timedBootStep('slash-autocomplete', initSlashCommandAutoComplete);
    }
    timedBootSyncStep('post-core-features', () => {
        initWorldInfo();
        initCfg();
        initReasoning();
        if (!isNoraProduct) {
    ...
    await timedBootStep('scrapers', initScrapers);
    timedBootSyncStep('final-ui-initializers', () => {
        initItemizedPrompts();
        if (!isNoraProduct) {
    ...
    await timedBootStep('app-initialized-event', () => eventSource.emit(event_types.APP_INITIALIZED));
    await timedBootStep('loader-hide', initLoaderHandle.hide);
    await timedBootStep('viewport', fixViewport);
    if (!isNoraProduct) {
        await criticalExtensionActivation;
    }
    await new Promise(resolve => setTimeout(resolve, 0));
    await timedBootStep('app-ready-event', () => eventSource.emit(event_types.APP_READY));
    noraBootMetrics.readyAt = Math.round((performance.now() - noraBootMetrics.startedAt) * 10) / 10;
    noraBootMetrics.milestones.push({ name: 'first-load-init-end', at: noraBootMetrics.readyAt });
    document.documentElement.dataset.noraAppReadyMs = String(noraBootMetrics.readyAt);
    console.info('[Nora boot] complete', structuredClone(noraBootMetrics));
    reportNoraBootMetrics('app-ready');
    void finishDeferredInitialization(criticalExtensionActivation).catch((error) => {
        document.body.classList.add('nora-extensions-failed');
        console.error('[Nora boot] Deferred initialization failed:', error);
    });
}
 ...
 *
 * If the character ID doesn't exist or the chat is being saved, this function does nothing.
 * If the character is different from the currently selected one, it clears the chat and selects it.
 ...
 * @param {string|null} [options.chatId=null] Chat to open as part of the character switch.
 * @param {boolean} [options.persistChat=true] Whether to persist the selected chat back to the character card.
 * @returns {Promise<void>} A promise that resolves when the character is switched.
 */
export async function selectCharacterById(id, { switchMenu = true, chatId = null, persistChat = true, activationSnapshot = null } = {}) {
    ...
    if (activationSnapshot) {
        await activateNoraWorldSnapshot(id, activationSnapshot);
        return;
    }
    ...
    if (String(this_chid) !== String(id)) {
        ...
        if (chatId && characters[id].chat !== chatId) {
            await openCharacterChat(chatId, { persistChat });
            return;
        }
...
export async function printCharacters(fullRefresh = false) {
    ...
    // Keep the character editor tags in sync when the list is rebuilt.
    applyTagsOnCharacterSelect();
    ...
    $('#rm_print_characters_pagination').pagination({
        ...
        callback: async function (/** @type {Entity[]} */ data) {
            ...
            const hidden = characters.length - displayCount;
...
export async function showMoreMessages(messagesToLoad = null) {
    ...
    const scrollElement = isNoraProductMode() ? $('#nora-chat') : chatElement;
    const prevHeight = scrollElement.prop('scrollHeight');
    ...
    if (isButtonInView) {
        const newHeight = scrollElement.prop('scrollHeight');
        scrollElement.scrollTop(scrollElement.scrollTop() + newHeight - prevHeight);
    }
...
/**
 * Send a message through the standard chat input pipeline.
 * @param {string|null} inputText Optional text supplied by a custom client.
 */
export async function sendTextareaMessage(inputText = null) {
    ...
    const hasDirectInput = typeof inputText === 'string';
    const textareaText = hasDirectInput ? inputText : String($('#send_textarea').val());
...
/**
 * Regenerate the latest assistant message through the standard single/group chat path.
 * @param {GenerateOptions} options Generation options for single-character chats.
 */
export async function regenerateLastMessage(options = {}) {
    ...
    is_send_press = true;
    return Generate('regenerate', options);
}
...
export function updateMessageElement(mes, { messageId = chat.length - 1, messageElement = messageTemplate.clone(), adjustMediaScroll = SCROLL_BEHAVIOR.NONE } = {}) {
    if (isNoraProductMode() && normalizeNoraReasoningMessage(mes, parseReasoningFromString)) saveChatDebounced();
    ...
    const avatarElement = messageElement.find('.avatar img');
    if (isNoraProductMode()) {
        avatarElement.removeAttr('src').attr('data-nora-avatar-src', avatarImg);
    } else {
        avatarElement.attr('src', avatarImg);
    }
 ...
 * @prop {number} [forceChId] Character ID to use for this generation run.
...
export function getCharacterCardFieldsLazy({ chid = undefined } = {}) {
    ...
    const resolvers = {
        persona: () => baseChatReplace(toCardText(power_user.persona_description).trim()),
        system: () => {
            if (!character) return '';
            const systemPrompt = toCardText(chat_metadata.system_prompt || character.data?.system_prompt);
            return power_user.prefer_character_prompt ? baseChatReplace(systemPrompt.trim()) : '';
        },
        jailbreak: () => {
            if (!character) return '';
            return power_user.prefer_character_jailbreak ? baseChatReplace(toCardText(character.data?.post_history_instructions).trim()) : '';
        },
        version: () => toCardText(character?.data?.character_version),
        charDepthPrompt: () => {
            if (!character) return '';
            return baseChatReplace(toCardText(character.data?.extensions?.depth_prompt?.prompt).trim());
        },
        creatorNotes: () => {
            if (!character) return '';
        ...
        mesExamples: () => {
            if (!character) return '';
            const exampleDialog = toCardText(chat_metadata.mes_example || character.mes_example);
            return baseChatReplace(exampleDialog.trim());
        },
        firstMessage: () => {
            if (!character) return '';
            const firstMes = toCardText(character.first_mes).trim();
        ...
        alternateGreetings: () => {
            ...
            return altGreetings.map(greeting => baseChatReplace(toCardText(greeting).trim()));
        },
    };
...
class StreamingProcessor {
    ...
    constructor(type, forceName2, timeStarted, continueMessage, promptReasoning) {
        ...
        this.swipes = [];
        /** @type {import('./scripts/logprobs-state.js').TokenLogprobs[]} */
        this.messageLogprobs = [];
        this.toolCalls = [];
        // Initialize reasoning in its own handler
        // New Nora streams begin after prompt/token preparation. Preserve ST's
        // accumulated continuation timing, but don't label local setup as thinking.
        const reasoningStarted = document.body.classList.contains('nora-product') && type !== 'continue'
            ? this.createdAt : timeStarted;
        this.reasoningHandler = new ReasoningHandler(reasoningStarted);
    ...
    onErrorStreaming() {
        ...
        this.isStopped = true;
        if (this.messageId >= 0 && this.type !== 'impersonate') {
            void this.reasoningHandler.finish(this.messageId).catch(error => console.warn('Could not finalize reasoning after stream failure:', error));
        }
    ...
    /**
     * @returns {AsyncGenerator<{ text: string, swipes: string[], logprobs: import('./scripts/logprobs-state.js').TokenLogprobs, toolCalls: any[], state: any }, void, void>}
     */
 ...
 * @property {number} [force_chid] Force character ID to use for the generation.
...
export async function Generate(type, { automatic_trigger, force_name2, quiet_prompt, quietToLoud, skipWIAN, force_chid, signal, quietImage, quietName, jsonSchema = null, depth = 0, inputText = null } = {}, dryRun = false) {
    ...
    if (!(dryRun || depth || type == 'regenerate' || type == 'swipe' || type == 'quiet')) {
        const commandInput = typeof inputText === 'string' ? inputText : String($('#send_textarea').val());
        const interruptedByCommand = await processCommands(commandInput);
    ...
    if (type !== 'regenerate' && type !== 'swipe' && type !== 'quiet' && !isImpersonate && !dryRun && !depth) {
        is_send_press = true;
        const hasDirectInput = typeof inputText === 'string';
        textareaText = hasDirectInput ? inputText : String($('#send_textarea').val());
        if (!hasDirectInput) {
            $('#send_textarea').val('')[0].dispatchEvent(new Event('input', { bubbles: true }));
        }
    } else {
    ...
    tagHistory(chat, scopeOf(chat_metadata));
...
export async function sendMessageAsUser(messageText, messageBias, insertAt = null, compact = false, name = name1, avatar = user_avatar) {
    // Hydration replaces chat, so it must precede the first write.
    if (isNoraProductMode()) await ensureNoraFullChatLoaded();
...
function parseAndSaveLogprobs(data, continueFrom) {
    /** @type {import('./scripts/logprobs-state.js').TokenLogprobs[] | null} */
    let logprobs = null;
...
export async function saveReply({ type, getMessage, fromStreaming = false, title = '', swipes = [], reasoning = '', imageUrls = [], reasoningSignature = null }) {
    ...
    if (type === 'swipe') {
        ...
        if (lastMessage.swipe_id === lastMessage.swipes.length - 1) {
            ...
            if (!fromStreaming && power_user.message_token_count_enabled) {
...
export function resetChatState() {
    ...
    chat.splice(0, chat.length, ...getSafetyChat());
...
export function setMenuType(value) {
    menu_type = value;
    document.getElementById('right-nav-panel')?.setAttribute('data-menu-type', menu_type);
}
 ...
 * @param {boolean} [options.skipBackup=false] Skip the recovery snapshot for an intermediate transaction save
 *
 * @returns {Promise<void>}
 */
...
/**
 * Collapses all saves performed by one Nora chat mutation into one recovery snapshot.
 * The canonical chat file is still written at every native ST save boundary.
 * @template T
 * @param {() => Promise<T>} operation Chat mutation operation.
 * @returns {Promise<T>}
 */
...
async function read_avatar_load(input) {
    if (input.files && input.files[0]) {
        ...
        if (!power_user.never_resize_avatars) {
            await globalThis.__NORA_ENSURE_OPTIONAL_UI__?.('cropper');
...
export async function getChat({ preloadedData = null, strict = false } = {}) {
    const snapshotStep = (suffix, operation) => preloadedData
        ...
        : operation();
    try {
        await snapshotStep('character-expand', () => unshallowCharacter(this_chid));

        const character = characters[this_chid];
        const normalizedChatName = String(character.chat || '').replace(/\.jsonl$/i, '');
        let data = preloadedData;
        if (data) {
            // Aggregate World activation already transferred this bounded chat window.
        } else if (isNoraProductMode()) {
            data = await requestNoraChatWindow();
        } else {
            const response = await fetch('/api/chats/get', {
                method: 'POST',
                headers: getRequestHeaders(),
        ...
        await snapshotStep('chat-install', async () => {
            if (isNoraProductMode() && !Array.isArray(data) && Array.isArray(data?.messages)) {
                const binding = currentNoraChatBinding();
                chat_metadata = data.header?.chat_metadata ?? {};
                chat.splice(0, chat.length, ...data.messages);
                chat.forEach(ensureMessageMediaIsArray);
                ...
                const chatHeader = legacyData.shift();
                chat_metadata = chatHeader?.chat_metadata ?? {};
                chat.splice(0, chat.length, ...legacyData);
                chat.forEach(ensureMessageMediaIsArray);
                noraChatWindowState = null;
            } else {
        ...
        if (preloadedData) {
            void snapshotStep('background.event.chat-loaded', emitChatLoaded)
                .catch(error => console.error('[Nora World] CHAT_LOADED listener failed:', error));
        } else void emitChatLoaded();
...
async function getChatResult({ snapshot = false } = {}) {
    ...
    await snapshotStep('event.chat-changed', () => eventSource.emit(event_types.CHAT_CHANGED, getCurrentChatId()));
    if (freshChat) await snapshotStep('event.chat-created', () => eventSource.emit(event_types.CHAT_CREATED));

    if (chat.length === 1) {
        const chat_id = (chat.length - 1);
        await snapshotStep('event.message-received', () => eventSource.emit(event_types.MESSAGE_RECEIVED, chat_id, 'first_message'));
        await snapshotStep('event.character-rendered', () => eventSource.emit(event_types.CHARACTER_MESSAGE_RENDERED, chat_id, 'first_message'));
    }
}
...
export async function openCharacterChat(file_name, { persistChat = true, preloadedData = null } = {}) {
    ...
    chat_metadata = {};
    await getChat({ preloadedData, strict: Boolean(preloadedData) });
...
/**
 * Activates a complete World snapshot through the existing ST chat lifecycle.
 * The aggregate response replaces network reads, not the native render/event owner.
...
export async function activateNoraWorldSnapshot(characterId, snapshot) {
    ...
    await timedBootStep('world.snapshot.runtime.previous-chat-clear', () => clearChat({ clearData: true }));
    cancelTtsPlay();
    this_edit_mes_id = undefined;
    selected_button = 'character_edit';
    timedBootSyncStep('world.snapshot.runtime.character-install', () => {
...
function activateMainAPIState(selectedVal) {
...
export function changeMainAPI(api = null, { applyLegacyUi = true } = {}) {
    const selectedVal = api ?? $('#main_api').val();
    if (!applyLegacyUi) {
        activateMainAPIState(selectedVal);
        return;
    }
...
//MARK: getSettings()
///////////////////////////////////////////
async function loadCompatibilitySettings(data, { applyLegacyUi, sourceSettings = settings } = {}) {
    const uiOptions = { applyLegacyUi };
    loadKoboldSettings(data, sourceSettings.kai_settings ?? sourceSettings, sourceSettings, uiOptions);
    loadNovelSettings(data, sourceSettings.nai_settings ?? sourceSettings, uiOptions);
    await loadTextGenSettings(data, sourceSettings, uiOptions);
    loadOpenAISettings(data, sourceSettings.oai_settings ?? sourceSettings, uiOptions);
    loadHordeSettings(sourceSettings, uiOptions);
    loadProxyPresets(sourceSettings, uiOptions);
}

async function loadActiveCompatibilitySettings(data, { applyLegacyUi }) {
    const uiOptions = { applyLegacyUi };
    const activeApi = settings.main_api === 'poe' ? 'openai' : (settings.main_api ?? 'kobold');

    switch (activeApi) {
        case 'openai':
            loadOpenAISettings(data, settings.oai_settings ?? settings, uiOptions);
            loadProxyPresets(settings, uiOptions);
            break;
        case 'textgenerationwebui':
            await loadTextGenSettings(data, settings, uiOptions);
            break;
        case 'novel':
            loadNovelSettings(data, settings.nai_settings ?? settings, uiOptions);
            break;
        case 'koboldhorde':
            loadHordeSettings(settings, uiOptions);
            break;
        case 'kobold':
        default:
            loadKoboldSettings(data, settings.kai_settings ?? settings, settings, uiOptions);
            break;
    }
}

async function loadRuntimeSettings(data, { applyLegacyUi }) {
    await timedBootStep('settings.identity-state', async () => {
    ...
    await timedBootStep('settings.event-before', () => eventSource.emit(event_types.SETTINGS_LOADED_BEFORE, settings));

    await timedBootStep('settings.runtime-core', async () => {
    ...
    await timedBootStep('settings.active-model-state', () => applyLegacyUi
        ? loadActiveCompatibilitySettings(data, { applyLegacyUi })
        : loadCompatibilitySettings(data, { applyLegacyUi: false }));
    await timedBootStep('settings.power-user-state', () => loadPowerUserSettings(settings, data, { applyLegacyUi }));
    await timedBootStep('settings.appearance-state', async () => {
        applyPowerUserSettings();
        loadTagsSettings(settings);
        loadBackgroundSettings(settings);
    });

    await timedBootStep('settings.event-after', () => eventSource.emit(event_types.SETTINGS_LOADED_AFTER, settings));

    await timedBootStep('settings.active-context', async () => {
        if (settings.main_api == undefined) settings.main_api = 'kobold';
        if (settings.main_api == 'poe') settings.main_api = 'openai';

        main_api = settings.main_api;
        changeMainAPI(main_api, { applyLegacyUi });
    ...
    await timedBootStep('settings.extension-config', async () => {
        if (data.enable_extensions) {
            const enableAutoUpdate = Boolean(data.enable_extensions_auto_update);
            const isVersionChanged = settings.currentVersion !== currentVersion;
            extensionActivationPlan = await loadExtensionSettings(settings, isVersionChanged, enableAutoUpdate, { deferActivation: true });
        } else {
...
export async function getSettings(initLoaderHandle = null) {
    const isNoraProduct = isNoraProductMode();
    const bundledSettings = isNoraProduct && globalThis.__NORA_SETTINGS_PROMISE__
        ? await globalThis.__NORA_SETTINGS_PROMISE__.catch(() => null)
        : null;
    let data = bundledSettings;
    if (!data) {
        const response = await timedBootStep('settings.fetch', () => fetch('/api/settings/get', {
            method: 'POST',
            headers: getRequestHeaders(),
            body: JSON.stringify({ scope: isNoraProduct ? 'runtime' : 'full' }),
            cache: 'no-cache',
        }));
        ...
        data = await timedBootStep('settings.decode-json', () => response.json());
    } else {
        noraBootMetrics.milestones.push({
            name: 'settings-bootstrap-reused',
            at: roundBootMetric(performance.now() - noraBootMetrics.startedAt),
        });
    }
    if (data.result != 'file not find' && data.settings) {
        await timedBootStep('settings.parse', async () => {
            settings = JSON.parse(data.settings);
        });

        await loadRuntimeSettings(data, { applyLegacyUi: !isNoraProduct });

        if (isNoraProduct) {
            firstRun = false;
        } else if (firstRun) {
            await timedBootStep('settings.onboarding', async () => {
    ...
    await timedBootStep('settings.disabled-samplers', validateDisabledSamplers);
    settingsReady = true;
    await timedBootStep('settings.loaded-event', () => eventSource.emit(event_types.SETTINGS_LOADED));
}

//MARK: saveSettings()
export async function saveSettings(loopCounter = 0, { strict = false } = {}) {
    if (!settingsReady) {
        if (strict) throw new Error('Settings are not ready for a confirmed save.');
    ...
    if (TempResponseLength.isCustomized()) {
        if (strict) throw new Error('Generation settings are temporarily overridden; save after generation completes.');
    ...
    if (isNoraProduct) {
        ...
        const activeProviderKeys = new Set(providerSettings[main_api] ?? providerSettings.kobold);
        for (const key of Object.values(providerSettings).flat()) {
            if (!activeProviderKeys.has(key)) delete payload[key];
        }
    }
    ...
    try {
        ...
        const settingsUrl = isNoraProduct ? '/api/settings/save?scope=runtime' : '/api/settings/save';
        const result = await fetch(settingsUrl, saveSettingsRequest);

        if (!result.ok) {
            throw new Error(`Failed to save settings: ${result.statusText}`);
        }

        settings = isNoraProduct ? { ...settings, ...payload } : payload;
...
export const saveSettingsStrict = () => saveSettings(0, { strict: true });
...
function applyMessageEdit(mes, inputText, { markTainted = true } = {}) {
    let text = inputText;
    ...
    if (markTainted) chat_metadata.tainted = true;
...
function updateMessage(div) {
    ...
    const inputText = mesBlock.find('.edit_textarea').val()
        ?? mesBlock.find('.mes_text').text();
    const mesElement = div.closest('.mes');
    const mes = chat[mesElement.attr('mesid')];
    return { mesBlock, ...applyMessageEdit(mes, inputText) };
}

/**
 * Commits a programmatic message edit through the same lifecycle as the native editor.
 * @param {number} messageId Message index in the active chat.
 * @param {string} text Replacement message text.
 * @returns {Promise<ChatMessage>}
 */
export async function commitMessageEdit(messageId, text) {
    if (scopeOf(chat_metadata)) return commitNoraStoryEdit(messageId, text);
    const mes = chat[messageId];
    if (!mes) {
        throw new Error(`Cannot edit missing message ${messageId}.`);
    }

    applyMessageEdit(mes, String(text ?? ''));
...
/** One authoritative edit/truncate commit, followed by the existing ST/MVU events. */
export async function commitNoraStoryEdit(messageId, text) {
    const absoluteId = getNoraAbsoluteMessageId(messageId);
    await ensureNoraFullChatLoaded();
    const oldLength = chat.length;
...
export async function messageEdit(editMessageId) {
    if (!ledgerAllowsEdit(editMessageId)) return;
...
async function messageEditDone(div) {
    if (scopeOf(chat_metadata) && this_edit_mes_id >= 0) {
        const value = div.closest('.mes_block').find('.edit_textarea').val();
        await commitNoraStoryEdit(this_edit_mes_id, String(value ?? ''));
        return;
    }
...
export function getCurrentChatDetails() {
    ...
    const currentChat = characters[this_chid].chat;
    const displayName = characters[this_chid].name;
    const avatarImg = getThumbnailUrl('avatar', characters[this_chid].avatar);
    return { sessionName: currentChat, group: null, characterName: displayName, avatarImgURL: avatarImg };
}
...
export async function setCharacterSettingsOverrides() {
    ...
    $template.find('[data-group="true"]').hide();
    $template.find('[data-character="true"]').show();
 ...
 * @param {Boolean} [options.syncUi=true] Whether to synchronize the legacy character browser controls
 * @returns {Promise<string>}
 */
export async function importCharacter(file, { preserveFileName = '', importTags = false, syncUi = true } = {}) {
    if (is_send_press) {
    ...
    if (syncUi) $('#character_import_file_type').val(format);
    ...
    try {
        ...
        if (data.file_name !== undefined) {
            ...
            if (syncUi) $('#character_search_bar').val('').trigger('input');
...
jQuery(async function () {
     ...
     * @returns {Promise<void>}
     */
    async function handleDeleteChat(chatFile, fromSlashCommand = false) {
    ...
    $('#options [id]').on('click', async function (event, customData) {
        ...
        if (id == 'option_select_chat') {
            ...
            await regenerateLastMessage(buildOrFillAdditionalArgs());
    ...
    $(document).on('input', '#curEditTextarea', function () {
        if (power_user.auto_save_msg_edits === true && !scopeOf(chat_metadata)) {
    ...
    $(document).on('click', '.mes .avatar', async function () {
        ...
        if ($(`.zoomed_avatar[forChar="${charname}"]`).length) {
            ...
            if (power_user.zoomed_avatar_magnification) {
                await globalThis.__NORA_ENSURE_OPTIONAL_UI__?.('avatarZoom');
