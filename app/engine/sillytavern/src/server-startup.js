...
// Express routers
import { router as userDataRouter } from './workspace.js';
...
import { router as noraBootRouter } from './endpoints/nora-boot.js';
...
import {
    compatibilityRouter as noraStoryProfileCompatibilityRouter,
    router as noraStoryProfileRouter,
} from './endpoints/nora-story-profile.js';
...
export function setupPrivateEndpoints(app) {
    app.use(createNoraTraceMiddleware());
    ...
    app.use('/', noraStoryProfileCompatibilityRouter);
...
export class ServerStartup {
    ...
    #getAddressInUseMessage(url, ipVersion) {
        ...
        return `Address ${listenAddress} is already in use. Another Nora Tavern instance may already be running. Stop the other process or change "port" in config.yaml.`;
    }
