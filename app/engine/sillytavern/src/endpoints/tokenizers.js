...
async function getPathToTokenizer(model, fallbackModel) {
    ...
    try {
        ...
        if (!arrayBuffer.byteLength) throw new Error('Downloaded tokenizer is empty');
        if (isCompressed) {
...
async function countOpenAiTokens(queryModel, messages) {
    let num_tokens = 0;
    const model = getTokenizerModel(queryModel);
...
router.post('/openai/count', async function (req, res) {
    ...
    try {
        const queryModel = String(req.query.model || '');
        const tokenCount = await countOpenAiTokens(queryModel, req.body);
        return res.send({ 'token_count': tokenCount });
    } catch (error) {
        console.error('An error counting tokens, using fallback estimation method', error);
        return res.send({ 'token_count': guesstimate(JSON.stringify(req.body)) });
    }
});

router.post('/openai/count-batch', async function (req, res) {
    const messages = req.body?.messages;
    if (!Array.isArray(messages)) return res.sendStatus(400);
    const queryModel = String(req.query.model || '');
    const tokenCounts = await Promise.all(messages.map(async message => {
        try {
            return await countOpenAiTokens(queryModel, [message]);
        } catch (error) {
            console.error('An error counting a batched message, using fallback estimation method', error);
            return guesstimate(JSON.stringify([message]));
        }
    }));
    return res.send({ 'token_counts': tokenCounts });
});
