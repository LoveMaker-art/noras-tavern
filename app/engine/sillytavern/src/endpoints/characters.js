...
class DiskCache {
     ...
     * @param {import('../workspace.js').UserDirectoryList[]} directoriesList List of user directories
     * @returns {Promise<void>}
     */
    ...
    return {
        ...
        data: {
            ...
            extensions: {
                ...
                nora_import: characterSourceMetadata(character),
            },
        },
    };
};
...
async function importFromPng(uploadPath, { request }, preservedFileName) {
    const sourceSha256 = sourceFingerprint(await fsPromises.readFile(uploadPath));
    ...
    attachCharacterSource(jsonData, sourceSha256);
    jsonData.name = sanitize(jsonData.data?.name || jsonData.name);
    const pngName = preservedFileName || getPngName(jsonData.name, request.user.directories);
    ...
    if (jsonData.spec !== undefined) {
        ...
        char = convertToV2(char, request.user.directories);
        attachCharacterSource(char, sourceSha256);
        const charJSON = JSON.stringify(char);
 ...
 *
 * Reads native character files, parses each card with `processCharacter`, and
 * returns the cards that contain a valid name.
 *
 * @param  {import("express").Request} request The HTTP request object.
 * @param  {import("express").Response} response The HTTP response object.
 * @return {void}
 */
export async function listCharacters(directories) {
...
/**
 * Reads one complete character card for a server-selected Nora World.
 * The avatar originates from the server-owned character/chat indexes, but it is
 * still reduced to a basename before touching the filesystem.
 * @param {import('../workspace.js').UserDirectoryList} directories User directories.
 * @param {string} avatar Character avatar filename.
 * @returns {Promise<object|null>} Complete character card or null when unavailable.
 */
export async function getCharacterByAvatar(directories, avatar) {
    const item = path.basename(String(avatar || ''));
    if (!item || item !== avatar || !fs.existsSync(path.join(directories.characters, item))) {
        return null;
    }
...
router.post('/all', async function (request, response) {
    try {
        return response.send(await listCharacters(request.user.directories));
    } catch (err) {
        console.error(err);
