...
router.post('/generate', async function (request, response) {
    ...
    try {
        ...
        const ledgerProof = request.body.nora_story_ledger;
        if (ledgerProof) {
            const ledgerRuntime = resolveStoryLedger(request.user.directories);
            await ledgerRuntime.resolve(ledgerProof);
            // Verify AFTER all provider/custom-body transformations, against the
            // exact payload about to be sent. Never forward proof metadata.
        ...
        if (request.body.stream) {
            if (fetchResponse.ok && ledgerDispatch) {
                await ledgerDispatch.accept();
                ledgerDispatch.release();
                response.set('X-Nora-Ledger-Activated', ledgerProof.recordId);
            }
