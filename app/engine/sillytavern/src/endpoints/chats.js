...
import { scopeOf } from '../../public/scripts/nora-story-ledger/history.js';
...
/**
 * Reads one bounded message window while preserving the chat header separately.
 * The server still owns the complete JSONL file; callers opt in so legacy clients
...
export function getChatWindowData(chatFilePath, { limit = 40, before } = {}) {
    ...
    const header = data[0] && typeof data[0] === 'object' ? data[0] : { chat_metadata: {} };
    const messages = data.slice(1);
    const boundedLimit = Math.min(100, Math.max(1, Number.parseInt(limit) || 40));
    const requestedBefore = before === undefined || before === null || before === ''
        ? messages.length
        : Number.parseInt(before);
    const end = Math.min(messages.length, Math.max(0, Number.isFinite(requestedBefore) ? requestedBefore : messages.length));
...
router.post('/get', validateAvatarUrlMiddleware, function (request, response) {
    try {
        ...
        if (request.body.nora_window) {
            return response.send(getChatWindowData(chatFilePath, {
                limit: request.body.nora_window,
...
router.post('/rename', validateAvatarUrlMiddleware, async function (request, response) {
    try {
        ...
        await resolveStoryLedger(request.user.directories).guardDestructive(pathToOriginalFile);
...
router.post('/delete', validateAvatarUrlMiddleware, async function (request, response) {
    try {
        ...
        await resolveStoryLedger(request.user.directories).guardDestructive(chatFilePath);
    ...
    try {
        ...
        if (format === 'json') {
            ...
            const handleChat = (chat) => {
                ...
                if (fs.existsSync(filePath)) throw new Error('Imported chat target already exists.');
...
router.post('/search', validateAvatarUrlMiddleware, async function (request, response) {
    try {
        const { query, avatar_url } = request.body;
        ...
        const chatFiles = fs.readdirSync(directoryPath)
...
export async function listRecentChats(directories, { max = Number.MAX_SAFE_INTEGER, metadata = false, pinned = [] } = {}) {
    ...
    const getCharacterChatFiles = async () => {
        ...
        for (const pngFile of pngFiles) {
            ...
            if (!fs.existsSync(pathToChats)) continue;
    ...
    const limit = parseInt(max ?? Number.MAX_SAFE_INTEGER) + pinnedChats.length;
    const isPinned = (/** @type {ChatFile} */ chatFile) => pinnedChats.some(item => item.file_name === path.basename(chatFile.filePath) && item.avatar === chatFile.pngFile);
    ...
    const jsonFilesPromise = recentChats.map(file => getChatInfo(file.filePath, { avatar: file.pngFile }, Boolean(metadata)));

    const chatData = (await Promise.allSettled(jsonFilesPromise)).filter(item => item.status === 'fulfilled').map(item => item.value);
    return chatData.filter(item => item.file_name);
}

router.post('/recent', async function (request, response) {
    try {
        return response.send(await listRecentChats(request.user.directories, {
