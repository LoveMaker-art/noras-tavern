...
export function normalizeTrace(payload = {}, user = 'unknown') {
    ...
    function clean(value, depth = 0) {
        ...
        if (value && typeof value === 'object') {
            ...
            return Object.fromEntries(entries.slice(0, 64).filter(([k]) => !SECRET.test(k))
...
export async function traceGeneration(request, response, next, {
    ...
    const isMvu = (Array.isArray(request.body.messages) && request.body.messages.some(m => typeof m?.content === 'string' && m.content.includes('Use nora-mvu/1.')))
    ...
    try {
        ...
        const allowed = ['model', 'chat_completion_source', 'stream', 'messages', 'tools', 'tool_choice', 'json_schema', 'response_format', 'temperature', 'max_tokens', 'max_completion_tokens'];
        save('model-request', Object.fromEntries(allowed.filter(k => k in request.body).map(k => [k, request.body[k]])));
        const decoder = new StringDecoder('utf8');
        const chunks = [], choices = new Map();
        let pending = '', totalBytes = 0, truncated = false, streaming = false, finished = false;
        ...
        function record(text) {
            ...
            let data;
            try { data = JSON.parse(text); } catch { data = { unparsed: text }; }
            ...
            if (Array.isArray(data.choices)) {
                for (const c of data.choices.slice(0, 8)) {
                    ...
                    const item = choices.get(key) ?? { index: key, content: '', tool_calls: [] };
        ...
        function complete(aborted) {
            ...
            try {
                ...
                save('model-response', { status: response.statusCode, durationMs: Date.now() - startedAt, aborted, streaming, totalBytes, choices: [...choices.values()], chunks }, truncated);
