 ...
 * @property {function(import('./server-startup.js').ServerStartupResult): URL} getRuntimeUrl Get the local runtime URL
 */
...
export class CommandLineParser {
    ...
    getDefaultConfig(isGlobal) {
        ...
        return Object.freeze({
            ...
            getRuntimeUrl: function () {
                throw new Error('getRuntimeUrl is not implemented');
            },
        });
    }
    ...
    parse(args) {
        ...
        const result = {
            ...
            getRuntimeUrl: function ({ useIPv6, useIPv4 }) {
                const hostname = useIPv4 ? '127.0.0.1' : (useIPv6 ? '[::1]' : '127.0.0.1');
                return new URL(
                    (this.ssl ? 'https://' : 'http://') +
                    (hostname) +
                    (':') +
