...
async function preSetupTasks() {
    ...
    console.log();
    console.log(`Nora Tavern ${version.pkgVersion}`);
...
async function postSetupTasks(result) {
    const runtimeUrl = cliArgs.getRuntimeUrl(result);
    ...
    setWindowTitle('Nora Tavern WebServer');

    let logListen = 'Nora Tavern is listening on';
    ...
    const goToLog = `Nora Tavern runtime: ${color.blue(runtimeUrl)}`;
    ...
    setupLogLevel();
    serverEvents.emit(EVENT_NAMES.SERVER_STARTED, { url: runtimeUrl });
}
