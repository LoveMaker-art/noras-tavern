...
// Respect the caller's slice, not the capacity of its backing ArrayBuffer.
var getView = (input, offset) => new DataView(input.buffer, input.byteOffset + offset, input.byteLength - offset);
...
function readBox(input, offset) {
  ...
  const encodedSize = readUInt32BE(input, offset);
  // ISO box size zero extends to EOF. Normalize it before returning a matching
  // box so every caller advances, including HEIF ispe and JPEG XL jxlp loops.
...
function readIFD(input, { isBigEndian, isBigTiff }) {
  ...
  const count = isBigTiff ? Number(readUInt64(input, ifdOffset, isBigEndian)) : readUInt(input, 16, ifdOffset, isBigEndian);
  const start = ifdOffset + entryCountSize;
  if (!Number.isSafeInteger(count) || count > Math.floor((input.length - start) / entrySize)) {
    throw new TypeError("Invalid TIFF directory bounds");
  }
  // The following IFD pointer / pixel bytes are not additional tag records.
  return input.slice(start, start + count * entrySize);
}
